...
def deform(problem_description, dm):
    ...
    for oth_alt in problem_description.alternativesSet:
        if oth_alt != dm_best_alternative:
            ...
            for j in cons:
                ...
                model.addConstr(quicksum([other_alternative_related_swapVar[oth_alt][(i_, j)] * dm.utilitiesList[i_] +
                                          E_ijVarDictPlus[oth_alt][(i_, j)] - E_ijVarDictMoins[oth_alt][(i_, j)]
            ...
            for i in pros:
                ...
                for j in cons:
                    ...
                    model.addConstr(
                        E_ijVarDictPlus[oth_alt][(i, j)] >= sig_i_plus - 1 + other_alternative_related_swapVar[oth_alt][
                            (i, j)])
                    model.addConstr(E_ijVarDictMoins[oth_alt][(i, j)] >= sig_i_moins - 1 +
    ...
    for oth_alt in problem_description.alternativesSet:
        if oth_alt != dm_best_alternative:
            ...
            pros, cons = set([i for i in range(len(array_dif)) if array_dif[i] == 1]), set(
                [i for i in range(len(array_dif)) if array_dif[i] == -1])
            Rm1 = {j: [i_ for i_ in pros if int(other_alternative_related_swapVar[oth_alt][(i_, j)].x) == 1] for j in
                   cons}
            print(oth_alt, Rm1)
