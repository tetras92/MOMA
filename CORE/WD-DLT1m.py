    ...
    for oth_alt in problem_description.alternativesSet:
        if oth_alt != dm_best_alternative:
            ...
            for i in pros:
                ...
                model.addConstr(w_i + sig_i_plus - sig_i_moins - quicksum([other_alternative_related_swapVar[oth_alt][(i, j_)]*dm.utilitiesList[j_] + E_ijVarDictPlus[oth_alt][(i, j_)] - E_ijVarDictMoins[oth_alt][(i, j_)] for j_ in cons]) >= local_epsilon)
           ...
           # linearisation
            for i in pros:
                for j in cons:
                    ...
                    model.addConstr(E_ijVarDictPlus[oth_alt][(i, j)] >= sig_j_plus - 1 + other_alternative_related_swapVar[oth_alt][(i, j)])
                    model.addConstr(E_ijVarDictMoins[oth_alt][(i, j)] >= sig_j_moins - 1 + other_alternative_related_swapVar[oth_alt][(i, j)])
