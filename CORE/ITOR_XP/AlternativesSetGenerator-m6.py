...
if __name__ == "__main__":
    ...
    LanguagesSelected = [decompose_under_L3]
    ...
    for repli in range(nb_replications):
        ...
        local_A_success_hn = 0
        local_A_success_h1 = 0
        for file in os.listdir(directory):
            ...
            for language in LanguagesSelected:
                ...
                if success_h1:
                    ...
                    local_A_success_h1 += 1

                success_hn, S_hn = recommendation_algorithm_any_tree_height(A, W, decomposition_function=language)
                if success_hn:
                    RESULT_Hn[language] += 1
                    local_A_success_hn += 1
        print(A, "hn", 100. * local_A_success_hn / len(os.listdir(directory)), "%", "h1", 100. * local_A_success_h1 / len(os.listdir(directory)), "%")

        #- Affichage
        for language in LanguagesSelected:
            print("n°", repli, language, "H=1", 100. * RESULT_H1[language] / (repli + 1) / len(os.listdir(directory)), "%")
            print("n°", repli, language, "H=n", 100. * RESULT_Hn[language] / (repli + 1) / len(os.listdir(directory)), "%")
