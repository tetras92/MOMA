...
def deform(problem_description, dm):
    ...
    # YZwVarDict = {(p1, p2): model.addVar(vtype=GRB.BINARY,
    #                                      name=f'b_{(dm_order_of_alternatives[p1], dm_order_of_alternatives[p2])}') for
    #               p1 in range(1, len(dm_order_of_alternatives) - 1)
    #               for p2 in range(p1 + 1, len(dm_order_of_alternatives))}
    # Eval_alt_after_deformation_list = [dm.evaluateAlternative(alt) + quicksum(
    #     [DeviationsSigma[k][0] for k in range(problem_description.n) if alt.attributeLevelsList[k] == 1]) - quicksum(
    #     [DeviationsSigma[k][1] for k in range(problem_description.n) if alt.attributeLevelsList[k] == 1])
    #                                    for alt in dm_order_of_alternatives]
    #
    # for (p1, p2), yzwVar in YZwVarDict.items():
    #     model.addConstr(
    #         Eval_alt_after_deformation_list[p1] - Eval_alt_after_deformation_list[p2] >= yzwVar - sum_w + local_epsilon)
    ...
    if model.status == GRB.OPTIMAL:
        # print("objVal", model.objVal, "/", (len(dm_order_of_alternatives) - 1) * (len(dm_order_of_alternatives) - 2) / 2,
        #       "pairs in YZw")
        for oth_alt in problem_description.alternativesSet:
            if oth_alt != dm_best_alternative:
                print(oth_alt, [(i, j) for (i, j), var_ij in other_alternative_related_swapVar[oth_alt].items() if
                                int(var_ij.x) == 1])

        print(model.objVal, [round(dm.utilitiesList[k] + DeviationsSigma[k][0].x - DeviationsSigma[k][1].x, 4) for k in
